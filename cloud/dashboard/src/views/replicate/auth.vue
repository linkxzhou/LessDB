  ...
  <div class="app-container">
    ...
    <el-table :data="tableList" :loading="listLoading" border style="width: 100%;top: -15px;">
      ...
      <el-table-column label="操作" width="200" align="center">
        <template slot-scope="scope">
          <el-button v-if="authType === 'target' && scope.row.status === 'accepted'" plain size="mini"
            class="filter-item" type="primary" @click="showReplicasForm(scope.row)">
            部署
          </el-button>
          <el-button v-if="authType === 'source' && scope.row.status !== 'accepted'" size="mini" plain type="primary"
            @click="handleUpdateAuth(scope.row)">接受</el-button>
          <el-button size="mini" plain type="danger" @click="handleDeleteAuth(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getReplicateAuths, createReplicateAuth, acceptReplicateAuth, deleteReplicateAuth, createReplicateRequest } from '@/api/console/replicate'
