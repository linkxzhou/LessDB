    ...
    <el-table :data="rolesList" style="width: 100%;margin-top:30px;" border>
      ...
      <el-table-column align="left" label="权限" min-width="300">
        <template slot-scope="{ row }">
          <el-tag v-for="perm in row.full_permissions" :key="perm._id" style="margin: 3px 6px">
            {{ perm.label }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作">
        <template slot-scope="scope">
          ...
          <el-button v-permission="'role.delete'" type="danger" size="small" @click="handleDelete(scope)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form :model="role" label-width="80px" label-position="left">
        ...
        <el-form-item label="角色权限">
          <el-select v-model="role.permissions" multiple placeholder="请选择角色" collapse-tags filterable>
            <el-option v-for="item in permissions" :key="item.name" :label="item.label" :value="item.name" />
          </el-select>
        </el-form-item>
