	...
	if typ.PkgPath() == "" {
		...
		case reflect.Int:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Int(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.int(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.int(ix) < vm.int(iy)) }
			}
		case reflect.Int8:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Int8(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.int8(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.int8(ix) < vm.int8(iy)) }
			}
		case reflect.Int16:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Int16(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.int16(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.int16(ix) < vm.int16(iy)) }
			}
		case reflect.Int32:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Int32(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.int32(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.int32(ix) < vm.int32(iy)) }
			}
		case reflect.Int64:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Int64(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.int64(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.int64(ix) < vm.int64(iy)) }
			}
		case reflect.Uint:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Uint(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.uint(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.uint(ix) < vm.uint(iy)) }
			}
		case reflect.Uint8:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Uint8(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.uint8(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.uint8(ix) < vm.uint8(iy)) }
			}
		case reflect.Uint16:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Uint16(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.uint16(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.uint16(ix) < vm.uint16(iy)) }
			}
		case reflect.Uint32:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Uint32(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.uint32(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.uint32(ix) < vm.uint32(iy)) }
			}
		case reflect.Uint64:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Uint64(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.uint64(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.uint64(ix) < vm.uint64(iy)) }
			}
		case reflect.Uintptr:
			if kx == kindConst && ky == kindConst {
				...
				y := xtype.Uintptr(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.uintptr(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.uintptr(ix) < vm.uintptr(iy)) }
			}
		case reflect.Float32:
			if kx == kindConst && ky == kindConst {
				...
				x := xtype.Float32(vx)
				return func(vm *goVm) { vm.setReg(ir, x < vm.float32(iy)) }
			} else if ky == kindConst {
				y := xtype.Float32(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.float32(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.float32(ix) < vm.float32(iy)) }
			}
		case reflect.Float64:
			if kx == kindConst && ky == kindConst {
				...
				x := xtype.Float64(vx)
				return func(vm *goVm) { vm.setReg(ir, x < vm.float64(iy)) }
			} else if ky == kindConst {
				y := xtype.Float64(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.float64(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.float64(ix) < vm.float64(iy)) }
			}
		case reflect.String:
			if kx == kindConst && ky == kindConst {
				...
				x := xtype.String(vx)
				return func(vm *goVm) { vm.setReg(ir, x < vm.string(iy)) }
			} else if ky == kindConst {
				y := xtype.String(vy)
				return func(vm *goVm) { vm.setReg(ir, vm.string(ix) < y) }
			} else {
				return func(vm *goVm) { vm.setReg(ir, vm.string(ix) < vm.string(iy)) }
			}
		}
	}
	panic("unreachable")
